      ...
      <div class="foryou">
        ...
        <ul class="tjforyou" v-if="exist">
          <li v-for="(m,i) in tuijian" :key="i" @click="getid(m.id)">
            ...
            <p class="price"><span>￥</span>{{m.price}}</p>
          </li>
        </ul>
...
<style lang="scss" scoped>
  ...
  .pic-list{
    width:100%;
    #iframe{
      width:100%;border:none;height:6000px;
    }
    .foryou{
      ...
      .lastitle{
        ...
        p{
          @include flex();text-align:center;height:.4rem;line-height: .4rem;
          &:nth-child(1).active{border-bottom: 2px solid #f00;}
          &:nth-child(2).active{border-bottom: 2px solid #f00;}
        }
      }
    };
    .tjforyou {
      @include flex-flow(row wrap);@include flex-direction();@include flexbox();
      width:100%;
      li{
        width:30%;margin:0 1.6%;margin-top:10px;
        img{width:100%;height:.9rem;border:1px solid #eee;}
        p{margin-top:5px;}
        .txt{
          font-size: 10px;height:.4rem;overflow:hidden;white-space:pre-wrap;text-overflow:ellipsis;
          line-height: .2rem;
        }
