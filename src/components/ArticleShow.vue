<template>
   <div id="art">
       ...
       <div id="other">
            <div id="top">
                <h1>{{result.title}}</h1>
                <p><span>{{result.createdate}}</span><i>{{result.classname}}</i></p>
            </div>
            <iframe :src="result.contenturl" srcolling="no" frameborder='0' id="iframe" ></iframe>
            <p id="category"><span>阅读&nbsp;{{result.hits}}</span><span>已发送&nbsp;{{result.sharenum}}</span><span @click="zan"><b :class="active" ></b>赞&nbsp;{{result.goodnum}}</span></p>
           <div id="cals">
                  <b id="move"></b> 
                <ul id="cal">
                <li @click="select(0)"><img src="../imgs/vp_kwz_tjwz.png"></img><span>推荐文章</span></li>
                <li @click="select(1)"><img src="../imgs/vp_kwz_zxph.png"></img><span>最新排行</span></li>
                <li @click="select(2)"><img src="../imgs/vp_kwz_ydph.png"></img><span>阅读排行</span></li>
                <li @click="select(3)"><img src="../imgs/kwz_search.png"></img></li>
                 </ul>
           </div>
           <ul id="calshow">
               <li v-for="(v,i) in res" :key="i"><p>{{ i+1 }}</p><span>{{v.title}}</span></li>
           </ul>
