<template>
    <div id="footer">
      <div id="sel" v-show="isshow">
        <p @click="wenzhang">看文章</p>
        <p @click="douzi">捡金豆</p>
      </div>
        <ul>
          <router-link to="/main/home" tag="li"  active-class="active" @click.native="hide" ><p /></router-link>
          <router-link to="/main/postage" tag="li"  active-class="active" @click.native="hide"><p /></router-link>
          <router-link to="/main/cart" tag="li"  active-class="active" @click.native="hide"><p /></router-link>
          <li  @click="check"><p /></li>
          <router-link to="/main/mine" tag="li"   active-class="active" @click.native="hide"><p/></router-link>
        </ul>
    </div>
</template>
...
export default {
  ...
  methods:{
    ...
    this.isshow = false;
    // console.log(123)
    this.$router.push('/money/2/article')
   },
   douzi(){
     this.isshow = false;
    // console.log(1234)
     this.$router.push('/money/3/gold')
   }
  },
 ...
 #footer{
   ...
   #sel{
    //  background: #0ff;
     ...
     height: .8rem;
     position: absolute;
     background: url('../imgs/w_popup_bg_money.png') no-repeat center center;
     background-size: 100%;
     bottom: .44rem;
     left: 60%;
