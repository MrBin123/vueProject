<template>
  <div id="gold">
    ...
    <div id="scroll">
      <div id="top">
        <mt-swipe :auto="4000">
          <mt-swipe-item v-for="(v,i) in result.propic" :key="i">
            <img v-lazy="v"></img>
          </mt-swipe-item>
        </mt-swipe>
      </div>
      <div id="name">
        <p>
          <span>广告名称：</span>{{result.title}}</p>
        <p>
          <span>广&nbsp;告&nbsp;语：</span>{{result.proword}}</p>
      </div>
      <div id="content">
        <span>内容介绍</span>
        <p>{{result.desc}}</p>
      ...
      <div id="goods">
        <p id="top">
          <b>商家兑换商品</b>
          <span>更多</span>
          <img src="../imgs/jjd_more.png"></img>
        </p>
        <ul>
          <li v-for="(v,i) in cont" :key="i">
            <img :src="v.picurl" id="leftcon">
            <img>
            <div id="rightcon">
              <p>{{v.goodsname}}</p>
              <b>
                <span>原价：{{v.price}}</span>
                <i>剩余：{{v.goodsnum}}件</i>
              </b>
              <i>
                <span>{{v.ylmjindou}}金豆</span>
                <b @click="exchange(v.id)">立即兑换</b>
              </i>
            </div>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    exchange: function (id) {
      //  console.log(id);
      this.$router.push({ name: "goodsshow", params: { id } });
    },
...
#gold {
  ...
  #scroll {
    @include flex();
    width: 100%;
    background: #efefef; // height: 400px;
    overflow-y: scroll;
    #top {
      width: 100%;
      height: 1.5rem;
